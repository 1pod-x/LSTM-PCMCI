...
| **CLSTM** | Sequence Recurrence | `inputs`: `[B, Input_Len, Total_Features]` | Initializes hidden states to $0$. Unrolls time sequence through `cell_stack` iteratively: $t \in [0, \text{Input\_Len})$. | `output`: `[B, 1]` | Models joint spatio-temporal causal dynamics and generates the final downstream target inference. |
